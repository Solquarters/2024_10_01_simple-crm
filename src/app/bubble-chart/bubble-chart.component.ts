...
//         this.cityCacheData = querySnapshot.docs.map((doc: any) => doc.data());
...
//       return (popA as number) - (popB as number); // ascending order
//     });
...
//     // Create a color scale from light yellow to dark blue
//     const colorScale = d3.scaleSequential<string>(d3.interpolateLab('#FEE08B', '#4575B4'))
...
//       cityColors.push({
//         city: city,
//         color: '#FFFFFF' // White color for unknown population
//       });
//     });
...
export class BubbleChartComponent implements OnInit, OnDestroy {
  ...
  private getColorScale(cityPopulationMap: Map<string, number | 'unknown'>, sortedCities: string[]): d3.ScaleOrdinal<string, string> {
    ...
    const colorScale = d3.scaleSequential<string>(d3.interpolateLab('#4575B4', '#FEE08B'))
      .domain([0, numKnownCities - 1]);
